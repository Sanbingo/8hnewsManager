    ...
    if (isArray(data) && data.length) {
      return data.filter(_ => !!_.middleURL).map(item => (
        <span style={{ position: 'relative', display: 'inline-block'}}>
          <img src={item.middleURL} style={{ width: '300px', height: '240px', cursor: 'pointer' }} onDoubleClick={(e) => {
            console.log('double Click', item)
            this.props.onOk(item.middleURL)
          }}/>
          <span style={{ position: 'absolute', left: 0, top: 0, padding: '0 2px', background: 'rgba(0,0,0,.6)', color: 'white'}}>{item.width}*{item.height}</span>
        </span>
      ))
    }
    return (
      <div>
        <p className={style.addIcon}>暂无图片</p>
      </div>
    )
  }
